...
type IssueFilterSpecMysql struct {
	repo.QuerySpec
	Filter queries.IssueFilter
}

func (s *IssueFilterSpecMysql) Apply(conn any) (any, error) {
	query := conn.(*gorm.DB)
	...
	if s.Filter.Search != nil {
		...
		if strings.Contains(rawSearch, ",") {
			...
			if len(iids) > 0 {
				query = query.Where(
					"(lower(issues.title) like ?) OR (issues.iid IN ?)",
			...
			query = query.Where(
				"(lower(issues.title) like ?) or (issues.iid = ?) OR (lower(issues.web_url) = ?)",
...
type IssueQueryImplMysql struct {
	queries.IssueQuery
}

func (b *IssueQueryImplMysql) GetSpec(filter queries.IssueFilter) repo.QuerySpec {
	return &IssueFilterSpecMysql{Filter: filter}
}
