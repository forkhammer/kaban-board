...
type IssueFilterSpecPostgresql struct {
	repo.QuerySpec
	Filter queries.IssueFilter
}

func (s *IssueFilterSpecPostgresql) Apply(conn any) (any, error) {
	query := conn.(*gorm.DB)
...
type IssueQueryImplPostgresql struct {
	queries.IssueQuery
}

func (b *IssueQueryImplPostgresql) GetSpec(filter queries.IssueFilter) repo.QuerySpec {
	return &IssueFilterSpecPostgresql{Filter: filter}
}
