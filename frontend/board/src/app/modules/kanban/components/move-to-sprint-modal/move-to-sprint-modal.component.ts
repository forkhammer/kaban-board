...
@Component({
  selector: 'app-move-to-sprint-modal',
  templateUrl: './move-to-sprint-modal.component.html',
  styleUrl: './move-to-sprint-modal.component.scss',
  standalone: false
})
export class MoveToSprintModalComponent {
  ...
  selectSprint(sprint: Sprint) {
    this.isLoading = true;
    this.issueBindingService.move(this.bindingId, sprint.id).pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
      next: result => {
